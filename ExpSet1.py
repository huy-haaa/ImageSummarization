...
def run_LS(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, p_root=0, trials=5, gph = True):
    ...
    for ii, kk in enumerate(k_vals_vec):
        ...
        for trial in range(trials):
            ...
            # Run the algorithm
            val, queries, time, sol, sol_r, time_r, queries_r, singletonVals = submodular_PGB.LinearSeq(objective, kk, eps, comm, rank, size, p_root, seed=trial );
            time_LS = time;
            val_LS = val;
...
def run_PGB(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, p_root=0, trials=5, gph = True):
    procs_vec = [size]*len(k_vals_vec)
    size_groundset = len([ele for ele in objective.groundset])
    n_vec = [size_groundset]*len(k_vals_vec)
    comm.barrier();
...
def run_FAST(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, p_root=0, trials=5, gph = True):
    procs_vec = [size]*len(k_vals_vec)
    size_groundset = len([ele for ele in objective.groundset])
    n_vec = [size_groundset]*len(k_vals_vec)
    comm.barrier()
...
def run_PLG(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, p_root=0, trials=1, gph = True):
    # Initialize data for saving csv files
    procs_vec = [size]*len(k_vals_vec)
    size_groundset = len([ele for ele in objective.groundset])
    n_vec = [size_groundset]*len(k_vals_vec)
    ...
    for trial in range(trials):
        ...
        # Run the algorithm
        valS_vec, queriesS_vec, timeS_vec = submodular_PGB.lazygreedy_parallel(objective, k_vals_vec, comm, rank, size);


        if rank == p_root:

            for i in range(len(valS_vec)):
                val = valS_vec[i]
                time = timeS_vec[i]
                queries = queriesS_vec[i]
                ii=i
                kk=k_vals_vec[i]
                val_vec.append(val)
                queries_vec.append(queries)
...
def run_LTLG_experiments(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, algo = "ALL", p_root=0, trials=5):
...
if __name__ == '__main__':
    ...
    k_frac = [0.001, 0.002, 0.004, 0.008, 0.016, 0.032, 0.064, 0.10]
    # ##################################################################
    # ##################################################################
    ...
    k_vals_vec = [100, 200, 400, 800, 1600, 3200, 6400, 10000]
    #k_vals_vec = [100]
    ...
    if(obj=="TRF"):
        ...
        k_vals_vec = [2, 4, 8, 16, 32, 64, 128, 188] # gets up to 72% Pems_Adj.csv"
    ...
    if(obj=="IFM"):
        ...
        k_vals_vec = [26, 52, 104, 208, 416, 932, 1864, 2658]
    ...
    if(obj=="RVM"):
        ...
        k_vals_vec = [17, 34, 68, 136, 272, 544, 1088, 1743]
    ...
    if(obj=="MVR"):
        ...
        try:
            ...
            k_vals_vec = [4, 8, 16, 32, 64, 128, 256, 370]
    ...
    if(obj=="IS_PLG"):
        ...
        k_vals_vec = [10, 20, 40, 80, 160, 320, 640, 1000]
        
        comm.barrier()
        run_LTLG_experiments(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, algo="PLG")
        # comm.barrier()
        # run_LTLG_experiments(objective, k_vals_vec, filepath_string, experiment_string, comm, rank, size, algo="LS")
        # comm.barrier()
   ...
   # ################################################################
    ...
    if(obj=="TS"):
        ...
        k_vals_vec = [42, 84, 168, 336, 672, 1344, 2688, 4205]
