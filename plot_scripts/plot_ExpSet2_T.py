...
for i in range( 0, nalgs ):
    ...
    ObjStd_tmp = []
    X_tmp = []
    # fname = "/Users/tonmoydey/Documents/Research_TheoreticalAlgo/Code/python-submodular/T/ER_100k_exp2_ABR_final.csv"
    fname = fnames[ i ];
    if (os.path.isfile( fname )):
        ...
        for j in range(len(nproc_distinct)):
            ...
            if(which==0):
                obj_ele = [elem for ii, elem in enumerate(f_of_S) if nproc_distinct[j] == nproc[ii]]
            if(which==1):
                obj_ele = [elem for ii, elem in enumerate(qry) if nproc_distinct[j] == nproc[ii]]
            if(which==3):
